...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (Schema::hasTable('settings') && Schema::hasTable('languages') ) :
            ...
            $timezone       = SettingsHelper::settingHelper('timezone');
            ...
            $appName        = SettingsHelper::settingHelper('application_name');

            if (!empty($appName)) :
                Config::set('app.name', $appName);
            endif;

            $captcha_sitekey        = SettingsHelper::settingHelper('captcha_sitekey');

            if (!empty($captcha_sitekey)) :
                Config::set('captcha.sitekey', $captcha_sitekey);
            endif;

            $captcha_secret        = SettingsHelper::settingHelper('captcha_secret');

            if (!empty($captcha_secret)) :
                Config::set('captcha.secret', $captcha_secret);
            endif;

            $default_storage = SettingsHelper::settingHelper('default_storage');
            ...
            if(!empty($default_storage)):
                ...
                $aws_access_key_id      = SettingsHelper::settingHelper('aws_access_key_id');
                $aws_secret_access_key  = SettingsHelper::settingHelper('aws_secret_access_key');
                $aws_default_region     = SettingsHelper::settingHelper('aws_default_region');
                $aws_bucket             = SettingsHelper::settingHelper('aws_bucket');
